...
function readsol(sol::Solution, filename::String)
    ...
    # du fichier lue
    if data.has_times       ; sol.cost = data.cost    end
    ...
    for (i,landing) in enumerate(data.landings)
        ...
        if data.has_times     ; sol.x[i]      = landing.time    end
        if data.has_times     ; sol.costs[i]  = landing.cost    end
    end
    ...
    Args.get("level")>=4 && println("readsol END")
    sol
end
...
function parse_solfile(inst::Instance, filename::String)
    ...
    while lid < length(lines)
        ...
        if key == "order"
            ...
            order_names = collect((m.match for m = eachmatch(r"[\w]+", val) ))
    ...
    if has_order && (sort(order_names) != sort(inst_names))
        msg = "ERREUR order : les noms de la ligne order ne correspondent " *
              "pas à l'instance."
    ...
    if has_landings && (sort(landing_names) != sort(inst_names))
        msg = "ERREUR landings : Les noms des lignes landing ne correspondent " *
              "pas à l'instance."
    ...
    if has_times && (cost != 0.0)
        ...
        if !isapprox(cost, sumcosts, atol=ATOL)
            msg = "ERREUR $(name) : coût global incorrect (atol=$ATOL): " *
